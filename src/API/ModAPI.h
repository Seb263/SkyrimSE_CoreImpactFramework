...
namespace CIF_API
{
	...
	public:
		...
		struct Filter
		{
			...
			{
				kAny, kDefault, kCloth, kLight, kHeavy,
				kWeaponDefault, kWeaponWood, kShieldHeavy, kShieldLight
			};

			enum class WeaponType
			{
				kAny, kOneHandSword, kTwoHandSword, kOneHandAxe, kTwoHandAxe,
				kOneHandMace, kTwoHandMace, kDagger, kRanged, kMagic,
				kHandToHand, kBeast, kOther
			};

			enum class AttackFilter { kAny, kRegular, kPower, kBash };
			enum class SourceFilter { kAny, kRightHand, kLeftHand, kDualHand, kShout, kOther };
			enum class StateFilter { kAny, kAlive, kDying, kKillmove, kDead };
			enum class ActorSex { kAny, kMale, kFemale };
			enum class BlockedFilter { kAny, kShieldHeavy, kShieldLight, kWeapon, kNo };
			enum class CriticalAttackFilter { kAny, kYes, kNo };
			enum class SneakAttackFilter { kAny, kYes, kNo };
		};
	...
	{
	public:
		virtual BipedEntry GetBipedEntry(RE::Actor* actor, const RE::NiPoint3& hitPosition) noexcept = 0;
	};
	
	class Interface_V3 : public Interface_V2
	{
	public:
		virtual void ProcessHit(RE::HitData& hitData, const bool runtimeReady) noexcept = 0;
		virtual void ProcessHit(RE::Actor* victim, RE::Projectile* projectile, const bool runtimeReady) noexcept = 0;
	};

	using Interface = Interface_V3;
