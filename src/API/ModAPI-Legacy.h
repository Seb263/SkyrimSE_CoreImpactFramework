...
SKSE::GetMessagingInterface()->RegisterListener([](MessagingInterface::Message* message) 
{
    switch (message->type) 
    {
        ...
        {
            if (!CoreImpactFrameworkAPI_Legacy::LoadAPI()) {
				util::report_and_fail("Failed to bound to the Core Impact Framework API");
			}
			CoreImpactFrameworkAPI_Legacy::g_API->GetVersion();
        }
        break;
    }
});
...
#define CIF_API_LEGACY_TYPE_KEY static_cast<uint32_t>(std::byteswap('CIF'))

// Define the API version in a structured format
#define CIF_API_LEGACY_VERSION_MAJOR 1
#define CIF_API_LEGACY_VERSION_MINOR 0
#define CIF_API_LEGACY_VERSION_PATCH 1

// Combine the version numbers into a single value
#define CIF_API_LEGACY_VERSION ((CIF_API_LEGACY_VERSION_MAJOR << 16) | (CIF_API_LEGACY_VERSION_MINOR << 8) | CIF_API_LEGACY_VERSION_PATCH)

namespace CoreImpactFrameworkAPI_Legacy
{
	using BipedBonesMap = std::optional<std::unordered_map<int, std::vector<std::string>>>;

	class CoreImpactFrameworkAPI_Legacy
	{
	public:
		// API functions
	...
	// Global API pointer
	inline extern CoreImpactFrameworkAPI_Legacy* g_API = nullptr;

	// Call this function only after the kDataLoaded event
	inline bool LoadAPI()
	{
		if (g_API != nullptr) return true;
		SKSE::GetMessagingInterface()->Dispatch(CIF_API_LEGACY_TYPE_KEY, (void*)&g_API, sizeof(void*), NULL);
		if (g_API) { // API successfully received!
			// Check if the API version matches
			return (g_API->GetAPIVersion() == CIF_API_LEGACY_VERSION);
		}
		return false;
	}
}
